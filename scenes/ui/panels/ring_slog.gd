...
func _ready():
	Events.connect("post_equipped_ring", on_post_equipped_ring)

func on_post_equipped_ring(ring):
	item_icon.item = ring if ring != null else null
	item_icon.texture = ring.icon if ring != null else null
...
func _can_drop_data(at_position, data): # Define se o dado pode ser dropado aqui, ou seja, o emissor também é receptor, ele pode transmitir para outros slots bem como receber.
	return data is TextureRect and data.item != null and data.item is Ring and r.get_requeriments(data.item)#or data.item is RefiningDust)
...
#	data.item._ready()
	...
	Events.emit_signal("ring_equipped", item_icon.item)
